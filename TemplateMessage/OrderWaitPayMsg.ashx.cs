...
{
    /// <summary>
    /// OrderWaitPayMsg 的摘要说明
    /// </summary>
    public class OrderWaitPayMsg : Enthos.UI.BaseHttpHandler
    {
        public override void DoAction(HttpContext context)
        {
            ...
            if (!string.IsNullOrEmpty(msgContent) && !string.IsNullOrEmpty(orderId) && !string.IsNullOrEmpty(proName) && !string.IsNullOrEmpty(startData))
            {
                ...
                string msg = msgCommad.GetMsgContent("sk-AbLQXRqsIp2FFzOm6rLiHZEAP2NU5qqH-XfSfcIA", openId, url, paramStr);
