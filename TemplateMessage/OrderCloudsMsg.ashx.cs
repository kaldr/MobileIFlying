...
{
    /// <summary>
    /// OrderCloudsMsg 的摘要说明
    /// </summary>
    public class OrderCloudsMsg : Enthos.UI.BaseHttpHandler
    {
        public override void DoAction(HttpContext context)
        {
            ...
            if (!string.IsNullOrEmpty(msgContent) && !string.IsNullOrEmpty(orderId) && !string.IsNullOrEmpty(proName) && !string.IsNullOrEmpty(startData))
            {
                ...
                string msg = msgCommad.GetMsgContent("EdY0ocTKtqSNaEiBukaDrgPha7NtHJH7XQWQVXxAYKc", openId, url, paramStr);
